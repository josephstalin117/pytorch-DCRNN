...
class DCRNNTrainer():
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            log = {'epoch': epoch}
            log.update({'loss': result['loss']})
            log.update({'train_mae': result['train_mae'][0]})
            log.update({'train_rmse': result['train_rmse'][0]})
            log.update({'train_mape': result['train_mape'][0]})
            log.update({'val_loss': result['val_loss']})
            log.update({'val_mae': result['val_mae'][0]})
            log.update({'val_rmse': result['val_rmse'][0]})
            log.update({'val_mape': result['val_mape'][0]})
    ...
    def _train_epoch(self, epoch):
        ...
        total_acc_rmse = np.zeros(1)
        total_acc_mae = np.zeros(1)
        total_acc_mape = np.zeros(1)
        ...
        for batch_idx, (data, target) in enumerate(self.data_loader.get_iterator()):
            ...
            #total_metrics += self._eval_metrics2(output.detach().numpy(), label.numpy())
            acc_mae, acc_rmse, acc_mape = eval_metrics(output.detach().numpy(), label.numpy())
        ...
        log = {
            ...
            'train_mae': total_acc_mae / self.len_epoch,
            'train_rmse': total_acc_rmse / self.len_epoch,
            'train_mape': total_acc_mape / self.len_epoch
    ...
    def _valid_epoch(self, epoch):
        ...
        return {
            ...
            'val_mae': total_acc_mae / self.val_len_epoch,
            'val_rmse': total_acc_rmse / self.val_len_epoch,
            'val_mape': total_acc_mape / self.val_len_epoch,
...
class DCRNNTester():
    ...
    def predict(self):
        ...
        test_mae, test_rmse, test_mape = eval_metrics(output.detach().numpy(), label.numpy())
        #test_mae = total_acc_mae/ self.test_len_epoch
        #test_rmse = total_acc_rmse / self.test_len_epoch
        #test_mape = total_acc_mape / self.test_len_epoch
        ...
        return test_loss, test_mae, test_rmse, test_mape, outputs, targets
...
def main(config):
    ...
    results = {"test": test_targets, "prediction": test_outputs, "train_loss": epoch_loss,
               "val_loss": val_loss, "rmse": test_rmse.tolist(), "mae": test_mae.tolist(), "mape": test_mape.tolist(), 
